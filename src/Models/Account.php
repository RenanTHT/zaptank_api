...
class Account extends Model {
    ...
    public function selectByUserAndPassword(string $email, $password) :array {
        ...
        $stmt = $conn->prepare("SELECT UserId, Password, IsBanned, VerifiedEmail, Email, Telefone, Opinion, IsFirstCharge FROM {$_ENV['BASE_SERVER']}.dbo.Mem_UserInfo WHERE Email = :email and Password = :password");
    ...
    public function selectById(int $userId) {
        ...
        $stmt = $conn->prepare("SELECT UserId, Password, IsBanned, VerifiedEmail, Email, Telefone, Opinion, IsFirstCharge FROM {$_ENV['BASE_SERVER']}.dbo.Mem_UserInfo WHERE UserId = :user_id");
    ...
    public function selectByEmail(string $email) {
        ...
        $stmt = $conn->prepare("SELECT UserId, Password, IsBanned, VerifiedEmail, Email, Telefone, Opinion, IsFirstCharge FROM {$_ENV['BASE_SERVER']}.dbo.Mem_UserInfo WHERE Email = :email");
		$stmt->bindParam(':email', $email);
		$stmt->execute();		
        return $result = $stmt->fetch(PDO::FETCH_ASSOC);
    }
    ...
    public function selectByPhone(string $phone) {
        ...
        $stmt = $conn->prepare("SELECT UserId, Password, IsBanned, VerifiedEmail, Email, Telefone, Opinion, IsFirstCharge FROM {$_ENV['BASE_SERVER']}.dbo.Mem_UserInfo WHERE Telefone = :phone");
